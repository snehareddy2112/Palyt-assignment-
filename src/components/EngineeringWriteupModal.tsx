...
export const EngineeringWriteupModal: React.FC<EngineeringWriteupModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl relative text-slate-900">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 rounded-t-3xl z-10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-extrabold text-slate-900">Engineering Write-up & Design Decisions</h3>
        ...
        {/* Content Body */}
        <div className="p-6 overflow-y-auto space-y-5 text-xs leading-relaxed text-slate-600">
          ...
          <section className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
            ...
            <div className="space-y-3">
              <div>
                <strong className="text-slate-900 block text-[13px]">Deleting Ingredients Used by Recipes:</strong>
                <p className="mt-0.5">
                ...
                <p className="mt-1 text-slate-500">
                  <span className="text-slate-800 font-semibold">Rationale:</span> In real restaurant operations, deleting an active ingredient corrupts recipe definitions and leads to runtime crashes during order calculation. Deletion must require either archiving or decoupling recipes first.
                </p>
              </div>

              <div>
                <strong className="text-slate-900 block text-[13px]">Mixed Unit Normalization Layer:</strong>
                <p className="mt-0.5">
              ...
              <div>
                <strong className="text-slate-900 block text-[13px]">Dataset Nuance Noted:</strong>
                <p className="mt-0.5">
          ...
          <section className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-white rounded-xl border border-slate-200 mt-2">
                <strong className="text-slate-900 block">What could still go wrong even if tests pass?</strong>
                <p className="mt-1 text-slate-500">
          ...
          <section className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-slate-700">
              <div className="p-3 bg-white rounded-xl border border-slate-200">
                <strong className="text-slate-900">1. Purchase Order Auto-Generation</strong>
                <p className="text-slate-500 mt-0.5">Automated supplier reorder draft when stock crosses below par levels.</p>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200">
                <strong className="text-slate-900">2. Wastage & Yield Tracking</strong>
                <p className="text-slate-500 mt-0.5">Account for kitchen prep wastage (e.g. peeling onions = 15% loss) before recipe deduction.</p>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200">
                <strong className="text-slate-900">3. Multi-portion Batch Ordering</strong>
                <p className="text-slate-500 mt-0.5">Table-level ticket management allowing batch preparation of multiple dishes.</p>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200">
                <strong className="text-slate-900">4. Recipe Editor</strong>
                <p className="text-slate-500 mt-0.5">Full CRUD UI for kitchen chefs to adjust portion sizes and add seasonal dishes.</p>
              </div>
            </div>
          </section>

          {/* AI Usage Disclosure */}
          <section className="border-t border-slate-100 pt-3 text-slate-400 text-[11px]">
            <strong className="text-slate-600">AI Usage Disclosure:</strong> AI tools were used to assist in structuring code, generating comprehensive Vitest test coverage, and crafting clean Tailwind styles. All business rules, unit normalization logic, and architectural decisions were verified and tested directly.
          </section>
        </div>

        {/* Footer */}
        <div className="px-6 py-3 border-t border-slate-100 bg-white rounded-b-3xl flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 text-xs font-bold text-white bg-[#FF7A00] hover:bg-[#E66E00] rounded-xl transition-colors cursor-pointer"
          >
            Close Write-up
          </button>
        </div>
